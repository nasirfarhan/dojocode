...
export default function ProjectTable({
  ...
  const handleDuplicateProject = async (project: Project) => {
    if (!onDuplicateProject) return false
    ...
    try {
      ...
      toast.error("Project dupliucation failed")
      console.error(error)
    }
    finally {
      setIsLoading(false)
          ...
          <TableBody>
            {projects.map((project) => (
              <TableRow key={project.id}>
                ...
                <TableCell>
                  <span className="text-sm text-grey-500">{format(new Date(project.createdAt), "MMM dd, yyyy")}</span>
                </TableCell>
                <TableCell>
